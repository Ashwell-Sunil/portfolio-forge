/**
 * Generates a complete, self-contained HTML page for a published portfolio.
 * Works on any device — no React, no external JS dependencies.
 */
const fs = require('fs');
...
const THEME_PRESETS = {
  'engineering-dark': {
    ...
    grid: true, glowOrbs: true, mono: false,
  ...
  'minimal-light': {
    ...
    grid: false, glowOrbs: false, mono: false,
  ...
  'editorial-glass': {
    ...
    grid: false, glowOrbs: true, mono: false, glass: true,
    displayFont: "'Space Grotesk',Inter,sans-serif", bodyFont: "Inter,system-ui,sans-serif",
  },
  'terminal-cyber': {
    ...
    grid: true, glowOrbs: false, mono: true, scanlines: true,
    displayFont: "'JetBrains Mono',monospace", bodyFont: "'JetBrains Mono','Courier New',monospace",
  },
};
...
function sectionHeading(text, t) {
  const prefix = t.mono ? `<span class="term-prefix" style="color:${t.accent}">▶</span>` : '';
  return `
    <div class="section-heading">
      ${prefix}
      <h2 style="color:${t.headingColor};font-family:${t.displayFont}">${esc(text)}</h2>
...
function generateCSS(t) {
  ...
  return `
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    ...
    .hero-orb1{position:absolute;top:0;right:0;width:280px;height:280px;border-radius:50%;background:radial-gradient(circle,rgba(${t.accentRgb},0.18),transparent 70%);transform:translate(30%,-30%);pointer-events:none}
    .hero-orb2{position:absolute;bottom:0;left:33%;width:200px;height:200px;border-radius:50%;background:radial-gradient(circle,rgba(${t.accentRgb},0.1),transparent 70%);transform:translateY(40%);pointer-events:none}
    .hero-inner{position:relative;display:flex;align-items:flex-start;justify-content:space-between;gap:32px}
    .hero-text{flex:1;min-width:0}
    .term-prefix-cmd{color:${t.titleColor};font-family:${t.bodyFont};font-size:.75rem;letter-spacing:.05em;margin-bottom:12px;opacity:.8}
    h1.name{font-family:${t.displayFont};font-weight:700;line-height:1.1;font-size:clamp(2rem,5vw,3.6rem);margin-bottom:12px;${
      t.nameGrad !== 'none'
        ? `background:${t.nameGrad};-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text`
        : `color:${t.nameColor||t.text}`
    };${t.mono ? `text-shadow:0 0 20px rgba(${t.accentRgb},0.5)` : ''}}
    ...
    .section{margin-bottom:48px;animation:fadeSlide .6s cubic-bezier(.16,1,.3,1) both}
    ...
    .badge{font-size:.8125rem;font-weight:600;padding:4px 10px;border-radius:6px;background:${t.badge};color:${t.badgeText};border:1px solid ${t.badgeBorder};white-space:nowrap;font-family:${t.mono?t.bodyFont:'inherit'}}
    ...
    @keyframes cardPop{from{opacity:0;transform:translateY(12px) scale(.97)}to{opacity:1;transform:translateY(0) scale(1)}}
    ...
    .cert-link,.cert-dl{font-size:.8125rem;color:${t.accent};display:inline-flex;align-items:center;gap:4px}
...
function renderPortfolio(data, themeId) {
  const { profile = {}, education = [], experience = [], projects = [], skills = [], certifications = [] } = data;
  const t = THEME_PRESETS[themeId] || THEME_PRESETS['engineering-dark'];
  const css = generateCSS(t);

  const avatarHtml = profile.imageUrl
    ? `<img src="${esc(profile.imageUrl)}" alt="${esc(profile.name)} photo" class="avatar">`
    : '';
        ...
        ${education.map(e => `
          <div class="edu-item">
            <div>
              ...
              <div class="edu-inst">${esc(e.institution || 'Institution')}</div>
              ${e.gpa ? `<div class="edu-gpa">GPA: ${esc(e.gpa)}</div>` : ''}
            </div>
            <span class="badge">${esc([e.startYear, e.endYear].filter(Boolean).join(' – ') || 'Year')}</span>
          </div>`).join('')}
      </div>
    </section>` : '';
          ...
          <div class="exp-item">
            ...
            <div class="exp-header">
              <div>
                ...
                <div class="exp-company">${esc(e.company || 'Company')}</div>
              </div>
      ...
      <div class="proj-grid${projects.length === 1 ? ' single' : ''}">
        ${projects.map(p => `
          <article class="proj-card${p.featured ? ' featured' : ''}">
            ${p.imageUrl ? `<img src="${esc(p.imageUrl)}" alt="${esc(p.title)} screenshot" class="proj-img">` : ''}
            <div class="proj-header">
              <div class="proj-title">${esc(p.title || 'Project')}</div>
            ...
            ${p.link ? `<a href="${esc(p.link)}" target="_blank" rel="noopener" class="proj-link">↗ ${t.mono ? '$ open project' : 'View Project'}</a>` : ''}
          </article>`).join('')}
      </div>
    </section>` : '';
  ...
  const certsHtml = certifications.length ? `
    <section class="section">
      ...
      ${certifications.map(c => `
        <div class="cert-item">
          <div>
            <div class="cert-name">${esc(c.name || 'Certification')}</div>
            ${c.issuer ? `<div class="cert-issuer">${esc(c.issuer)}</div>` : ''}
          </div>
          <div class="cert-meta">
            ${c.year ? `<span class="cert-year">${esc(c.year)}</span>` : ''}
            ${c.fileUrl ? `<a href="${esc(c.fileUrl)}" target="_blank" class="cert-dl">📎 View</a>` : ''}
  ...
  const slug = profile.slug || (profile.name || '').toLowerCase().replace(/[^a-z0-9]/g,'-').replace(/-+/g,'-').slice(0,30) || 'portfolio';
  const fontsUrl = `https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&family=Space+Grotesk:wght@400;500;600;700&family=JetBrains+Mono:wght@400;500;600&display=swap`;
...
<head>
  ...
  <meta property="og:description" content="${esc(profile.about?.slice(0, 160) || `Portfolio of ${profile.name}`)}">
  ${profile.imageUrl ? `<meta property="og:image" content="${esc(profile.imageUrl)}">` : ''}
...
<body>
  <article>
    <header class="hero">
      ...
      ${t.glowOrbs ? '<div class="hero-orb1"></div><div class="hero-orb2"></div>' : ''}
      <div class="hero-inner">
        <div class="hero-text">
          ${t.mono && profile.name ? `<div class="term-prefix-cmd">$ whoami</div>` : ''}
...
function embedLocalAssets(html) {
  const regex = /(?:src|href)="(\/uploads\/([^"]+))"/g;
  return html.replace(regex, (match, url, filename) => {
    try {
      ...
      if (fs.existsSync(filePath)) {
        ...
        return match.replace(url, `data:${mime};base64,${base64}`);
      }
    } catch (e) {
      console.error('Failed to embed', url, e);
    }
    return match;
  });
}

module.exports = { renderPortfolio, embedLocalAssets };
